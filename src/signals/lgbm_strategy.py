...
class LGBMOptunaStrategy(Strategy):
    def __init__(self, n_trials=50, n_splits=10, feature_set="macro_"):
    ...
    def fit(self, X, y):
        ...
        def objective(trial):
            ...
            tscv = TimeSeriesSplit(n_splits=self.n_splits, max_train_size=max_train_size, test_size=6, gap=gap)
            scores = []
    ...
    def generate_signal(self, past_data, current_data):
        ...
        for _, X_pred in X_preds.iterrows():
            ...
            if normalized_signal > 0:
                ...
                pred = 0
            preds.append(pred)
            amounts.append(min(abs(normalized_signal) * 10, 10))
